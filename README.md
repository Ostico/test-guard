...
- **Gemini 3.x cannot disable thinking.** `minimal` is the floor, and it "matches the no-thinking setting for most queries" without guaranteeing it. `gemini-3.1-flash-lite` defaults to `minimal`; Gemini 3 Flash defaults to `high`, which risks a thought trace consuming Layer 3's 2048-token output budget.
